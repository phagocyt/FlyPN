...
def get_zscores(df, num_bin=20):
    ...
    for _, group in df.groupby("mean_bin"):
        ...
        index_w_nan = group.index
        index_wo_nan = index_w_nan.dropna()
        df.loc[index_wo_nan, "z_score"] = z_scores
...
class PCA():
    ...
    def plot_loadings(self, ax, component=0, num_genes=20):
        ...
        ax.barh(list(range(len(plot_data))), plot_data, align="center")
        ax.set_xlabel("PC " + str(component + 1) + " Loading")
        yticklabels = plot_data.index
        ax.set_yticks(list(range(len(plot_data))))
...
class TSNE():
    ...
    def calc_TSNE(self, perplexity=30, random_state=0, learning_rate=500.0, early_exaggeration=4.0, metric="correlation", n_iter=1000, method="barnes_hut"):
        if metric=="correlation":
            ...
            self.dist = np.clip(self.dist, 0.0, float(np.nanmax(self.dist.values)))
    ...
    def plot(self, fig, ax, colorBy=None, colorMode="gene", **kwargs):
        ...
        if colorMode == "gene":
            ...
            sc = ax.scatter(self.X_tsne[:,0], self.X_tsne[:,1], s=80, edgecolor="k", linewidths=0.3 ,c=C, norm=norm, **kwargs)
        else:
            sc = ax.scatter(self.X_tsne[:,0], self.X_tsne[:,1], s=80, edgecolor="k", linewidths=0.3 ,c=C, **kwargs)
        ...
        if colorMode == "gene" and singleColor == False:
            ...
            for i, feature in zip(color_tuple_indexes, colorBy):
                c = [1., 1., 1.]
                c[i] = 0.
                ax.scatter(max(self.X_tsne[:,0]) + 1e6, 0, s=50, linewidths=0.1 , c=c, label=feature)
...
class hclust:
    ...
    def cut(self, n_clusters_genes=2, n_clusters_cells=2):
        ...
        X1 = self.X[self.X.columns[list(map(bool, 1-cell_labels))]]
        X2 = self.X[self.X.columns[list(map(bool, cell_labels))]]

        genes1 = list(self.X.T[self.X.T.columns[list(map(bool, 1-gene_labels))]].T.index)
        genes2 = list(self.X.T[self.X.T.columns[list(map(bool, gene_labels))]].T.index)
...
class ICIM:
    ...
    def calc(self, skip=[], verbose=False):
        ...
        if verbose:
            print()
        # this line is added by QX to speed up iteration...
        num_markers = 0
        while (len(queue) != 0) & (num_markers<1200):
            ...
            if verbose:
                print()
            # this line is added by QX to speed up iteration...
            num_markers = len(set([item for sublist in list(self.markers.values()) for item in sublist]))

            if num_markers >= 2000:
                print('Stoping because ICIM genes found is greater than 2000.')

        return None

    def get_all_markers(self):
        allMarkers = list(set([item for sublist in list(self.markers.values()) for item in sublist]))
        return allMarkers
